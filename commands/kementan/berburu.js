...
module.exports = {
    ...
    async execute(sock, msg, args) {
        ...
        if (rng < 10) { 
            ...
            // --- UPDATE 1: PENGHASILAN BERBURU SESUAI RARITY & LEVEL ---
            const rarityMultiplier = {
                'Common': 1, 'Rare': 1.5, 'Epic': 2.5, 
                'Mythic': 4, 'Legendary': 7, 'SECRET': 12, 'Unknown': 1
            };
            const pengali = rarityMultiplier[petTarget.rarity || 'Common'] || 1;
            
            // Batasi reward agar tidak mencetak uang melebihi kas bank
            let nexusDapat = Math.floor((Math.random() * 200 + 50) * pengali) + (petTarget.level * 20);
            nexusDapat = Math.min(nexusDapat, Math.max(0, global.db.bank.brankas));
