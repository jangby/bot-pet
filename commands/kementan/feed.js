...
module.exports = {
    ...
    async execute(sock, msg, args) {
        ...
        let petTarget = kandang.find(p => p.id == petId); // Menggunakan == agar fleksibel
        let isPetBersama = false;

        if (!petTarget && global.db.petBersama) {
            const petAnak = global.db.petBersama.find(p => p.id == petId && (p.ortu1 === senderNumber || p.ortu2 === senderNumber));
            if (petAnak) {
                petTarget = petAnak;
                isPetBersama = true;
            }
        ...
        if (!petTarget) {
            return await sock.sendMessage(chatId, { text: `⚠️ Peliharaan dengan ID [${petId}] tidak ditemukan di kandangmu maupun panti asuhan bersama.` }, { quoted: msg });
        }
        ...
        // --- UPDATE 2: XP MAKANAN BERDASARKAN HARGA/KUALITAS ---
        // Hitung XP berdasarkan harga modal makanan (Harga dibagi 10, minimal 2 XP)
        const hargaMakanan = infoMakanan.modal || 50;
        const baseXP = Math.max(2, Math.floor(hargaMakanan / 10));
        
        // Tambahkan gacha sedikit biar angkanya tidak kaku
        const xpMakan = baseXP + Math.floor(Math.random() * 10);
        // --- UPDATE 6: SISTEM ITEM BUFF ---
        let efekBuff = '';
        if (idMakananDitemukan === 'elixir_murni') {
            petTarget.buff = 500; // Nambah power +500 untuk 1x duel
            efekBuff = `\n\n🔥 *EFEK BUFF AKTIF!*\nElixir Murni bereaksi! Power ${petTarget.nama} meningkat +500 untuk 1x duel berikutnya!`;
        } else if (idMakananDitemukan === 'buah_surga') {
            petTarget.buff = 200; // Nambah power +200
            efekBuff = `\n\n⚡ *EFEK BUFF AKTIF!*\nBuah Surga memberikan energi magis! Power ${petTarget.nama} meningkat +200 untuk 1x duel berikutnya!`;
        }
