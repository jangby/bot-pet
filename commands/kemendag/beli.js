...
module.exports = {
    ...
    async execute(sock, msg, args) {
        ...
        if (uangPembeli < totalHarga) return await sock.sendMessage(chatId, { text: `⚠️ Uangmu tidak cukup! Tagihan: *${totalHarga.toLocaleString('id-ID')} Nexus*.` });
        
        // --- SISTEM PAJAK PPN 10% ---
        const pajakPPN = Math.floor(totalHarga * 0.1);
        const pendapatanBersih = totalHarga - pajakPPN;

        // Distribusi Uang
        global.db.player[buyerNumber].saldo -= totalHarga; // Pembeli bayar penuh
        global.db.player[sellerNumber].saldo = (parseInt(global.db.player[sellerNumber].saldo) || 0) + pendapatanBersih; // Penjual dipotong pajak
        global.db.bank.brankas = (global.db.bank.brankas || 0) + pajakPPN; // Uang pajak masuk Kas Negara

        itemEtalase.stok -= jumlahBeli;
        dataToko.pendapatan = (dataToko.pendapatan || 0) + pendapatanBersih;
        ...
        const teksPembeli = `✅ *TRANSAKSI BERHASIL*\n\nPembelian di *${dataToko.nama}* sukses.\n📦 ${jumlahBeli}x ${infoBarangInduk.nama}\n💰 Total Harga: *${totalHarga.toLocaleString('id-ID')} Nexus*\n\n🏛️ _(Pajak PPN 10% sebesar ${pajakPPN.toLocaleString('id-ID')} Nexus telah ditarik oleh Negara)_`;
